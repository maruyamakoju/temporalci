...
def _estimate_required_pairs(
    *,
    ...
    return estimate_required_pairs(
        alpha=alpha,
        beta=beta,
...
def _run_calibrate_with_args(args: argparse.Namespace) -> int:
    ...
    summary: dict[str, Any] = {
        ...
        "tool": {
            "name": "temporalci.sprt_calibration",
            "version": __version__,
        },
        "exit_code_semantics": {
            "0": "ok",
            "1": "input_or_runtime_error",
            "2": "check_failed",
        },
        "generated_at_utc": utc_now_iso(),
        "suite_path": str(Path(args.suite).resolve()),
        "suite_hash_sha1": _sha1_file(suite_path),
...
def run_apply_from_calibration(
    *,
    ...
    if out is not None and inplace:
        print("--inplace and --out are mutually exclusive")
        return 1
    ...
    target_metric = gate_metric or str(summary.get("gate_metric", "")).strip()
    if not target_metric:
        print("--gate-metric is required when calibration json has no gate_metric")
        return 1
    ...
    if inplace:
        ...
        apply_out = Path(out).resolve()
    else:
        apply_out = suite_path.with_suffix(".calibrated.yaml")

    applied_path, apply_diff = _apply_recommended_params_to_suite(
        suite_path=suite_path,
        gate_metric=target_metric,
        recommended_params=recommended,
        apply_out=apply_out,
        apply_inplace=inplace,
    )
    print(
        "applied_calibration "
        f"path={applied_path} "
...
def run_check_from_calibration(
    *,
    calibration_json: str | Path,
    fail_if_no_deltas: bool = False,
    min_total_deltas: int | None = None,
...
def sprt_main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="temporalci sprt",
        description="SPRT calibration/apply/check utilities",
    )
    ...
    apply_cmd.add_argument("--calibration-json", required=True)
    ...
    if args.sprt_command == "check":
        return run_check_from_calibration(
            calibration_json=args.calibration_json,
            fail_if_no_deltas=bool(args.fail_if_no_deltas),
            min_total_deltas=args.min_total_deltas,
            max_mismatch_runs=args.max_mismatch_runs,
    ...
    print(f"unknown sprt command: {args.sprt_command}")
    return 1
